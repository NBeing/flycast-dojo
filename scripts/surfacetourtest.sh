...
# travel visibly (a 600 ms drag under a large pointer) - `[2026-09-20]` "i didnt see a lot of the
# hand intent tour and then the movie played at like really fast speed".
WATCHCFG=(); [ "$WATCH" -eq 1 ] && WATCHCFG=(-config dojo:TourRealtime=yes -config "dojo:HandGestureMs=${HAND_GESTURE_MS:-600}")
XDG_CONFIG_HOME="$OUT/cfg" XDG_DATA_HOME="$OUT/data" DISPLAY="$D" "$EXE" \
	...
	-config "dojo:HandStrokes=${HAND_STROKES:-}" -config "dojo:HandStop=${HAND_STOP:-0}" -config "dojo:HandPeak=${HAND_PEAK:-0}" -config "dojo:HandTapeSlot=${HAND_TAPE_SLOT:--1}" "${WATCHCFG[@]}" ${TOUR_EXTRA_CFG:-} \
