...
void VulkanContext::CreateSwapChain()
{
	try
	{
		...
		do {
			...
			// swapchain: for the automation screenshot, for Syphon video
			// routing, and for video capture. It costs nothing when unused, so
			// it is requested unconditionally rather than under the narrower
			// TEST_AUTOMATION / VIDEO_ROUTING guards this base had.
...
void VulkanContext::Present() noexcept
{
	if (renderDone)
	{
		try {
			DoSwapAutomation();
			// Must read the composited image BEFORE it is presented - this is
			// the only point where the frame the user actually sees, ImGui and
			// Lua overlays included, exists as one image.
			DoSwapCapture();
