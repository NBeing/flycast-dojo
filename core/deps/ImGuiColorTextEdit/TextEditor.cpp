...
#include <string>
#include <set>
#include <regex>	// flycast-dojo: std::regex instead of vendored boost
...
void TextEditor::SetLanguageDefinition(LanguageDefinitionId aValue)
{
	...
	case LanguageDefinitionId::Hlsl:
		...
		break;
	case LanguageDefinitionId::TasNotation:	// flycast-dojo
		mLanguageDefinition = &(LanguageDefinition::TasNotation());
		break;
	case LanguageDefinitionId::Ppad:		// flycast-dojo
		mLanguageDefinition = &(LanguageDefinition::Ppad());
		break;
	}
...
{
	mLineSpacing = Max(0.7f, Min(2.0f, aValue));	// flycast-dojo: below 1.0 allowed - the notepad packs its rows (dev: too much padding)
}
...
void TextEditor::SelectPreviousOccurrenceOf(const char* aText, int aTextSize, bool aCaseSensitive)	// flycast-dojo
{
	ClearSelections();
	ClearExtraCursors();
	SelectPreviousOccurrenceOf(aText, aTextSize, -1, aCaseSensitive);
}
...
bool TextEditor::Render(const char* aTitle, bool aParentIsFocused, const ImVec2& aSize, bool aBorder)
{
	...
	ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(3.0f, 2.0f));	// flycast-dojo: less air around the text (the theme pads 8px)
	...
	if (mRequestFocus) { ImGui::SetWindowFocus(); mRequestFocus = false; }	// flycast-dojo: Tab panel-swap focus (NoNavInputs child, so SetKeyboardFocusHere cannot reach it)
	ImGui::SetWindowFontScale(mFontScale);	// flycast-dojo: text zoom
...
void TextEditor::SelectPreviousOccurrenceOf(const char* aText, int aTextSize, int aCursor, bool aCaseSensitive)	// flycast-dojo
{
	if (aCursor == -1)
		aCursor = mState.mCurrentCursor;
	Coordinates prevStart, prevEnd;
	if (FindPreviousOccurrence(aText, aTextSize, mState.mCursors[aCursor].mInteractiveStart, prevStart, prevEnd, aCaseSensitive))
	{
		SetSelection(prevStart, prevEnd, aCursor);
		EnsureCursorVisible(aCursor, true);
	}
}
...
void TextEditor::AddCursorForNextOccurrence(bool aCaseSensitive)
{
	...
	{
		// flycast-dojo: VSCode behavior - the FIRST Ctrl+D selects the word
		// under the cursor (upstream just returned, so Ctrl+D looked dead
		// unless a selection already existed). Falls back one column so it
		// also works with the cursor at the word's right edge.
		Coordinates at = GetSanitizedCursorCoordinates();
		Coordinates ws = FindWordStart(at);
		Coordinates we = FindWordEnd(at);
		if (!(ws < we) && at.mColumn > 0)
		{
			const Coordinates left = { at.mLine, at.mColumn - 1 };
			ws = FindWordStart(left);
			we = FindWordEnd(left);
		}
		if (!(ws < we))
			return;
		SetSelection(ws, we, mState.GetLastAddedCursorIndex());
		return;	// the NEXT press adds the next occurrence
	}
...
bool TextEditor::FindPreviousOccurrence(const char* aText, int aTextSize, const Coordinates& aFrom, Coordinates& outStart, Coordinates& outEnd, bool aCaseSensitive)
{	// flycast-dojo: mirror of FindNextOccurrence, walking BACKWARD (find-previous)
	assert(aTextSize > 0);
	int fline, ifline;
	int findex, ifindex;
	...
	while (true)
	{
		// move backward FIRST - we want a match strictly before aFrom
		if (findex == 0)
		{
			if (fline == 0)
			{
				fline = (int)mLines.size() - 1;
				findex = (int)mLines[fline].size();
			}
			else
			{
				fline--;
		...
		{ // match function (identical forward scan starting at findex)
			int lineOffset = 0;
			int currentCharIndex = findex;
			int i = 0;
			for (; i < aTextSize; i++)
...
// flycast-dojo: append text at the very end WITHOUT resetting the undo
// history (SetText nukes it) - the notepad's panel-evening rides this.
void TextEditor::SetGutterLabels(std::vector<GutterLabel> aLabels)	// flycast-dojo
{
	mGutterLabels = std::move(aLabels);
	mGutterMaxLen = 0;
	for (const auto& g : mGutterLabels)
		mGutterMaxLen = std::max(mGutterMaxLen, (int)g.mText.size());
}

void TextEditor::AppendLines(const std::string& aText)
{
	if (aText.empty() || mReadOnly)
		return;
	UndoRecord u;
	u.mBefore = mState;
	const int lastLine = (int)mLines.size() - 1;
	const Coordinates insertAt = { lastLine, GetLineMaxColumn(lastLine) };
	Coordinates where = insertAt;
	InsertTextAt(where, aText.c_str());
	u.mOperations.push_back({ aText, insertAt, where, UndoOperationType::Add });
	u.mAfter = mState;
	AddUndo(u);
	Colorize(insertAt.mLine, (int)mLines.size() - insertAt.mLine + 2);
}

// flycast-dojo: how many distinct LINES the selections cover - the user's
// wait-counting trick (select the blank run, read the number, that is the
// frames to wait when programming the move).
int TextEditor::GetSelectedLineCount() const
{
	std::unordered_set<int> selLines;
	for (int c = 0; c <= mState.mCurrentCursor; c++)
	{
		const Cursor& cur = mState.mCursors[c];
		if (!cur.HasSelection())
			continue;
		const int a = cur.GetSelectionStart().mLine;
		int b = cur.GetSelectionEnd().mLine;
		if (cur.GetSelectionEnd().mColumn == 0 && b > a)
			b--;	// a selection ending AT a line start does not cover that line
		for (int l = a; l <= b; l++)
			selLines.insert(l);
	}
	return (int)selLines.size();
}

// flycast-dojo: VSCode Ctrl+Alt+Up/Down - clone the last cursor one line
// up/down (column clamped to the target line), the multi-cursor column setup.
void TextEditor::AddCursorAboveOrBelow(bool aBelow)
{
	// clone from the EXTREME ACTIVE cursor (top for Up, bottom for Down), not the "current" one - otherwise the
	// 2nd press duplicated the same cursor and MergeCursorsIfPossible dropped it (the "works once" bug). Repeated
	// presses now extend the stack, VSCode-style.
	// After the sort the top-most active cursor is index 0 and the bottom-most is index mCurrentCursor. Use those
	// indices, NOT mCursors.back()/.front(): ClearExtraCursors (Esc) only lowers mCurrentCursor without shrinking
	// mCursors, so .back() could read a STALE cursor parked at a line END from a prior op - its column then landed
	// the new cursor on the LAST column of the next line instead of the same column (the reported bug).
	mState.SortCursorsFromTopToBottom();
	const Cursor& ext = aBelow ? mState.mCursors[mState.mCurrentCursor] : mState.mCursors[0];
	const Coordinates from = aBelow ? ext.GetSelectionEnd() : ext.GetSelectionStart();
	const int line = from.mLine + (aBelow ? 1 : -1);
	if (line < 0 || line >= (int)mLines.size())
		return;
	const int col = Min(from.mColumn, GetLineMaxColumn(line));
	mState.AddCursor();
	SetCursorPosition({ line, col }, mState.GetLastAddedCursorIndex(), true);
	mState.SortCursorsFromTopToBottom();
	MergeCursorsIfPossible();
	EnsureCursorVisible();
}

// flycast-dojo: VSCode Shift+Alt+Up/Down. Duplicates the contiguous span the
// cursors/selections cover, inserting the copy right below it; aMoveCursorDown
// shifts the cursors onto the copy (Down flavor) so repeated presses propagate.
void TextEditor::DuplicateCurrentLines(bool aMoveCursorDown)
{
	assert(!mReadOnly);
	...
	const Coordinates insertAt = { maxLine, GetLineMaxColumn(maxLine) };
	const std::string block = "\n" + GetText({ minLine, 0 }, insertAt);
	Coordinates where = insertAt;
	InsertTextAt(where, block.c_str());
	u.mOperations.push_back({ block, insertAt, where, UndoOperationType::Add });

	if (aMoveCursorDown)
	{
		const int span = maxLine - minLine + 1;
		for (int c = mState.mCurrentCursor; c > -1; c--)
		{
			mState.mCursors[c].mInteractiveStart.mLine += span;
			mState.mCursors[c].mInteractiveEnd.mLine += span;
		}
	}
	u.mAfter = mState;
	AddUndo(u);
	Colorize(minLine, (maxLine - minLine + 1) * 2 + 2);
	EnsureCursorVisible();
}
...
void TextEditor::HandleKeyboardInputs(bool aParentIsFocused)
{
	if (ImGui::IsWindowFocused() || aParentIsFocused)
	{
		...
		if (!mReadOnly && isShortcut && ImGui::IsKeyPressed(ImGuiKey_Z))
			...
			MoveEnd(shift);
		else if (!mReadOnly && !alt && !super && ImGui::IsKeyPressed(ImGuiKey_Delete))	// flycast-dojo: Shift+Delete = Delete (typing habit)
			Delete(ctrl);
		else if (!mReadOnly && !alt && !super && ImGui::IsKeyPressed(ImGuiKey_Backspace))	// flycast-dojo: Shift+Backspace = Backspace (typing habit)
			Backspace(ctrl);
		...
		// flycast-dojo: the VSCode bindings - Alt moves, Shift+Alt duplicates
			...
			DuplicateCurrentLines(true);
		else if (!mReadOnly && alt && !ctrl && shift && !super && ImGui::IsKeyPressed(ImGuiKey_UpArrow))
			DuplicateCurrentLines(false);
		else if (alt && ctrl && !shift && !super && ImGui::IsKeyPressed(ImGuiKey_UpArrow))
			AddCursorAboveOrBelow(false);
		else if (alt && ctrl && !shift && !super && ImGui::IsKeyPressed(ImGuiKey_DownArrow))
			AddCursorAboveOrBelow(true);
		// flycast-dojo: three-mod fallback - many Windows GPU drivers register
		// Ctrl+Alt+Arrow globally (screen rotation) and eat it before any app
		else if (alt && ctrl && shift && !super && ImGui::IsKeyPressed(ImGuiKey_UpArrow))
			AddCursorAboveOrBelow(false);
		else if (alt && ctrl && shift && !super && ImGui::IsKeyPressed(ImGuiKey_DownArrow))
			AddCursorAboveOrBelow(true);
		// flycast-dojo: arrow-free duplicate fallback (three-key arrow combos can
		// be eaten by drivers or blocked by keyboard matrices)
		else if (!mReadOnly && isShiftShortcut && ImGui::IsKeyPressed(ImGuiKey_D))
			DuplicateCurrentLines(true);
			...
			EnterCharacter('\n', false);
		// Tab is NOT bound to indent here: the flycast TAS Notepad (the only user of
		// this editor) uses Tab to swap the P1/P2 track - see tasNotepadWindow.
...
void TextEditor::Render(bool aParentIsFocused)
{
	...
	const float gutterFontSize = ImGui::GetFontSize() * mGutterScale;	// flycast-dojo: the gutter can be smaller than the text
	const float gutterGap = fontWidth * 0.5f;							// flycast-dojo: half a character between gutter and text
	if (mShowLineNumbers)
	{
		snprintf(lineNumberBuffer, 16, " %zu", mLines.size());
		mTextStart += ImGui::GetFont()->CalcTextSizeA(gutterFontSize, FLT_MAX, -1.0f, lineNumberBuffer, nullptr, nullptr).x + gutterGap;
	}
	if (mShowLineNumbers && mGutterMaxLen > 0)	// flycast-dojo: custom gutter labels can be wider than the line numbers
		mTextStart = std::max(mTextStart, mLeftMargin + mGutterMaxLen * fontWidth * mGutterScale + gutterGap);
	...
	if (!mLines.empty())
	{
		...
		std::string hoverText;	// flycast-dojo: the diagnostics / decorations under the mouse this frame (one tooltip after the loop)
		...
		for (int lineNo = mFirstVisibleLine; lineNo <= mLastVisibleLine && lineNo < mLines.size(); lineNo++)
		{
			...
			// flycast-dojo: full-row highlight for the active movie frame's line (drawn under selection + text)
			if (lineNo == mHighlightLine)
			...
			// Draw line number (right aligned) - or the caller's gutter label for this line (flycast-dojo)
			if (mShowLineNumbers)
			{
				const GutterLabel *gl = lineNo < (int)mGutterLabels.size() && !mGutterLabels[lineNo].mText.empty() ? &mGutterLabels[lineNo] : nullptr;
				std::string gutterText;
				if (gl != nullptr)
					gutterText = gl->mText;
				else
				{
					snprintf(lineNumberBuffer, 16, "%d", lineNo + mLineNumberBase);
					gutterText = lineNumberBuffer;
				}
				const float lineNoWidth = ImGui::GetFont()->CalcTextSizeA(gutterFontSize, FLT_MAX, -1.0f, gutterText.c_str(), nullptr, nullptr).x;
				drawList->AddText(ImGui::GetFont(), gutterFontSize,
						ImVec2(lineStartScreenPos.x + mTextStart - gutterGap - lineNoWidth, lineStartScreenPos.y + (ImGui::GetFontSize() - gutterFontSize) * 0.5f),
						gl != nullptr && gl->mColor != 0 ? gl->mColor : mPalette[(int)PaletteIndex::LineNumber], gutterText.c_str());
			}
			...
			while (charIndex < mLines[lineNo].size() && column <= mLastVisibleColumn)
			{
				...
				else
				{
					...
					if ((mBoldMask & (1u << (int)glyph.mColorIndex)) != 0 && !glyph.mComment && !glyph.mMultiLineComment)
						drawList->AddText(ImVec2(targetGlyphPos.x + 1.0f, targetGlyphPos.y), color, glyphBuffer.c_str());	// flycast-dojo: faux bold
				}

				MoveCharIndexAndColumn(lineNo, charIndex, column);
			}

			// flycast-dojo: diagnostics + decorations - a zigzag / a bar / nothing under a char range, the messages on hover
			for (int list = 0; list < 2; list++)
				for (const auto& d : list == 0 ? mDiagnostics : mDecorations)
				{
					if (d.mLine != lineNo)
						continue;
					const int lineLen = (int)line.size();
					const int i0 = Max(0, Min(d.mStart, lineLen)), i1 = Max(i0, Min(d.mEnd, lineLen));
					const int c0 = GetCharacterColumn(lineNo, i0), c1 = GetCharacterColumn(lineNo, i1);
					const float x0 = textScreenPos.x + TextDistanceToLineStart({ lineNo, c0 }, false);
					float x1 = textScreenPos.x + TextDistanceToLineStart({ lineNo, c1 }, false);
					if (x1 < x0 + mCharAdvance.x)
						x1 = x0 + mCharAdvance.x;	// an empty range still shows one glyph's worth
					const float yBase = lineStartScreenPos.y + ImGui::GetFontSize() - 1.0f;
					if (d.mStyle == DecoStyle::Squiggle)
					{
						const float amp = 1.5f, step = 3.0f;
						ImVec2 prev(x0, yBase + amp);
						bool up = true;
						for (float x = x0 + step; x <= x1 + 0.5f; x += step, up = !up)
						{
							const ImVec2 p(x, up ? yBase - amp : yBase + amp);
							drawList->AddLine(prev, p, d.mColor, 1.0f);
							prev = p;
						}
					}
					else if (d.mStyle == DecoStyle::Bar)
						drawList->AddRectFilled(ImVec2(x0, yBase + 1.0f), ImVec2(x1, yBase + 3.0f), d.mColor);
					if (!d.mMessage.empty() && ImGui::IsWindowHovered()
							&& ImGui::IsMouseHoveringRect(ImVec2(x0, lineStartScreenPos.y), ImVec2(x1, lineStartScreenPos.y + mCharAdvance.y)))
					{
						if (!hoverText.empty())
							hoverText += '\n';
						hoverText += d.mMessage;
					}
				}
		}
		if (!hoverText.empty())
		{	// wrapped: a long message used to run across the whole screen (dev)
			ImGui::BeginTooltip();
...
// flycast-dojo: the per-line buffer start, visible to tokenize callbacks so
// they can judge a token by its LINE position (which side of a '|', a leading
// P1/P2 prefix) - the player-true coloring rides this.
const char *TextEditor_tokenizeLineBegin = nullptr;
...
void TextEditor::ColorizeRange(int aFromLine, int aToLine)
{
	...
	for (int i = aFromLine; i < endLine; ++i)
	{
		...
		TextEditor_tokenizeLineBegin = bufferBegin;	// flycast-dojo
