...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            {
                g.NavDisableHighlight = true;
                // [FLYCAST PATCH] Check if dragging (except for scrollbars)
                if (held && !pressed)
                {
                    ...
                    if (ImAbs(delta.x) >= 5.f || ImAbs(delta.y) >= 5.f)
                    {
                        ...
                        {
                            // ClearActiveID only HERE: the original patch cleared it for EVERY
                            // held drag, which killed dock-splitter drags after 5px (the
                            // "resize barely moves" jank) - splitters are ButtonBehavior too.
                            ClearActiveID();
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (window->DragScrolling)  // [FLYCAST PATCH]
    {
        selected = false;
