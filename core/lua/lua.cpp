 ...
 * game pixels and land on the game image. That image is not the window: it is
 * letterboxed into whatever area the UI leaves for it, which shrinks when tool
 * windows are docked.
 *
 * This does not re-derive that rectangle, it reads the one the renderers place
 * the picture with (core/rend/game_viewport.h). It used to be a second copy of
 * renderLastFrame()'s arithmetic, kept in step by hand - so when docking made
 * the window stop being the picture, both copies were wrong at once. Now there
 * is one derivation and a script's overlay tracks the docked layout for free.
 */
static int getGameViewport(lua_State *L)
{
	const rend::ViewportRect vp = rend::gameViewport();
	lua_pushnumber(L, vp.x);
	lua_pushnumber(L, vp.y);
	lua_pushnumber(L, vp.w);
	lua_pushnumber(L, vp.h);
	return 4;
}
...
//! has taken space compares the two.
static int getWindowSize(lua_State *L)
{
	lua_pushinteger(L, settings.display.width);
	lua_pushinteger(L, settings.display.height);
	return 2;
}
		...
		.beginNamespace ("flycast")
	  		.beginNamespace("emulator")
				...
				.addFunction("isReplay", std::function<bool()>([]() {
					return dojo.play_match;
				}))
				//! Frames in the replay being played back, or recorded so far.
				//
				// dojo-7 rename map for this binding:
				//   PlayMatch        -> play_match
				//   maple_inputs     -> session_inputs   (the re-record map:
				//                       frame -> packet, last-write-wins, which
				//                       is what makes in-place re-recording work)
				//   recording_replay -> recording_started
