...
*/
void Watch::arm(u64 frames, double now)
{
...
Verdict Watch::check(u64 frames, double now, bool running)
{
	std::lock_guard<std::mutex> lock(mtx_);
	if (!armed_)
...
void livenessSelfTest()
{
	...
	{	// AN UNARMED WATCH SAYS NOTHING - paired with the positive, so a class
		...
		Watch w = fresh();
		const Verdict idle = w.check(100, 10.0, true);
		w.arm(100, 10.0);
		const Verdict after = w.check(105, 10.1, true);	// frames moved
	...
	{	// THE MACHINE MOVED. One completed frame is enough - the question is
		...
		claim("a single completed frame is alive",
				w.check(501, 20.5, true) == Verdict::Alive);
	}
	{	// STILL INSIDE THE GRACE. A load takes real time; calling it dead at
		// once would fire on every healthy load.
		Watch w = fresh();
		w.arm(500, 20.0);
		const Verdict early = w.check(500, 21.9, true);	// no movement, 1.9s < 2.0
		const Verdict late  = w.check(500, 22.1, true);	// no movement, past grace
	...
	{	// REPORTS ONCE. A dead machine is dead every frame after, and thousands
		...
		Watch w = fresh();
		w.arm(500, 20.0);
		const Verdict first  = w.check(500, 23.0, true);
		const Verdict second = w.check(500, 24.0, true);
	...
	{	// AND ALIVE DISARMS. Otherwise a machine that ran and later paused
		...
		Watch w = fresh();
		w.arm(500, 20.0);
		const Verdict alive = w.check(501, 20.1, true);
		const Verdict later = w.check(501, 99.0, true);	// paused, long after
	...
	{	// A SECOND LOAD RE-ARMS. Every load gets its own verdict.
		Watch w = fresh();
		w.arm(500, 20.0);
		w.check(501, 20.1, true);				// alive, disarmed
		w.arm(600, 30.0);				// loaded again
		claim("a second load is judged on its own",
				w.armed() && w.check(600, 32.1, true) == Verdict::Dead);
	}
	...
	{	// A PAUSED MACHINE IS NOT A DEAD ONE. The normal TAS case: load a state
		...
		Watch w = fresh();
		w.arm(500, 20.0);
		const Verdict held = w.check(500, 99.0, false);		// paused 79s > grace
	...
	{	// AND THE CLOCK RESTARTS WHEN IT RESUMES. This is the claim that
		...
		Watch w = fresh();
		w.arm(500, 20.0);
		w.check(500, 50.0, false);							// paused for 30s
		const Verdict justAfter = w.check(500, 51.0, true);	// 1s of running < 2.0
		const Verdict then      = w.check(500, 52.1, true);	// 2.1s of running
