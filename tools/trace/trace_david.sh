...
echo "frame combo p1A_state p1A_atk p1B_state p1B_atk p1C_state p1C_atk p1A_hitstop p2A_state p2A_hitstop p2A_air skipTog skipCnt dist"
# slot addresses: A = base, B = +0xB48, C = +0x1690 (the 0x5A4 stride x2 ... use the SPREADSHEET's per-slot columns instead)
declare -A A=( [combo]=0x2C2685A0 [p1A_state]=0x2C268510 [p1A_atk]=0x2C2684E1 [p1B_state]=0x2C269058 [p1B_atk]=0x2C269029 [p1C_state]=0x2C269BA0 [p1C_atk]=0x2C269B71 [p1A_hitstop]=0x2C2684E0 [p2A_state]=0x2C268AB4 [p2A_hitstop]=0x2C268A84 [p2A_air]=0x2C268ADD [skipTog]=0x2C289622 [skipCnt]=0x2C289621 )
while :; do
  r=$(send $SEQ query '{}'); SEQ=$((SEQ+1)); f=$(field "$r" frame); [ "$f" -gt "$TO" ] && break
  line="$f"; for k in combo p1A_state p1A_atk p1B_state p1B_atk p1C_state p1C_atk p1A_hitstop p2A_state p2A_hitstop p2A_air skipTog skipCnt; do rd ${A[$k]}; line="$line $RDV"; done
  a=$(send $SEQ read '{"addr":"0x2C2685D8","width":4}'); SEQ=$((SEQ+1)); dv=$(field "$a" value); line="$line $(python3 -c "import struct;print(round(struct.unpack('<f',struct.pack('<I',$dv))[0]))" 2>/dev/null)"
  echo "$line"
